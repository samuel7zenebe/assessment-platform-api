...
export const getQuestionById = factory.createHandlers(sValidator("param", z.object({ id: z.uuid() })), async (c) => {
...
export const softDeleteQuestion = factory.createHandlers(sValidator("param", z.object({ id: z.string().min(1) })), async (c) => {
    const { id } = c.req.valid("param");
    try {
...
export const deleteQuestion = factory.createHandlers(sValidator("param", z.object({ id: z.string().min(1) })), async (c) => {
    const { id } = c.req.valid("param");
    try {
