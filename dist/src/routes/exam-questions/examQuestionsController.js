...
import { exams, examJobTitles, questionBank, questionJobTitles, } from "@/src/db/schema.js";
import { APIError } from "better-auth";
import z from "zod";
import { eq } from "drizzle-orm";
import { sValidator } from "@hono/standard-validator";
import { ExamQuestionSchema, ReorderQuestionSchema, BulkReorderSchema, } from "./schema.js";
const factory = createFactory();
// ── POST  /exams/:examId/questions         → attach question ─────────────────
export const addQuestionToExam = factory.createHandlers(sValidator("param", z.object({
    id: z.uuid(),
})), sValidator("json", ExamQuestionSchema.pick({
    questionId: true,
    questionOrder: true,
})), async (c) => {
    ...
    try {
        ...
        const data = await db.transaction(async (tx) => {
            ...
            const [questionData] = await db
                .select()
                .from(questionBank)
                .leftJoin(questionJobTitles, eq(questionJobTitles.questionId, questionBank.id))
                .leftJoin(examJobTitles, eq(examJobTitles.jobTitleId, questionJobTitles.jobTitleId))
...
export const bulkReorder = factory.createHandlers(sValidator("json", BulkReorderSchema), async (c) => {
...
export const updateExamQuestion = factory.createHandlers(sValidator("json", ReorderQuestionSchema), async (c) => {
    const examId = c.req.param("examId");
    const { questionId, questionOrder } = c.req.valid("json");
    try {
        ...
        const updated = await examQuestionsRepo.updateExamQuestion({ examId, questionId }, { questionOrder });
        return c.json({
            data: updated,
            success: true,
...
export const removeQuestionFromExam = factory.createHandlers(sValidator("json", ReorderQuestionSchema), async (c) => {
    const examId = c.req.param("examId");
...
export const getExamQuestions = factory.createHandlers(async (c) => {
    const examId = c.req.param("examId");
    try {
        const questions = await examQuestionsRepo.getExamQuestionsWithDetails(examId);
