...
import { examAttempts, examQuestions, attemptQuestions as attemptQuestionsTable, answers as answersTable, questionBank, questionChoices, exams, } from "@/src/db/schema.js";
...
export const attemptsRepo = {
    ...
    createAttemptWithSnapshots: async (params) => {
        ...
        const [lastAttempt] = await db
            ...
            .from(examAttempts)
            .where(and(eq(examAttempts.examId, params.examId), eq(examAttempts.candidateId, params.candidateId)))
        ...
        const insertData = rawQuestions.map((eqRow, idx) => {
            ...
            if (!question)
                throw new Error(`Question not found: ${eqRow.questionId}`);
            const choices = choicesMap[eqRow.questionId] || [];
            return {
                ...
                questionSnapshot: {
                    ...
                    choices: question.type === "CHOICE" || question.type === "TRUE_FALSE"
                        ? choices.map((c) => ({
                            id: c.id,
                            choiceKey: c.choiceKey,
                            choiceText: c.choiceText,
    ...
    getAttemptWithQuestions: async (attemptId) => {
        const [attempt] = await db
            .select()
            .from(examAttempts)
        ...
        const filteredQuestions = questions.map((aq) => {
            ...
            if (snapshot.choices && Array.isArray(snapshot.choices)) {
                snapshot.choices = snapshot.choices.map((c) => {
    ...
    getQuestionByOrder: async (attemptId, order) => {
        const [aq] = await db
            .select()
            .from(attemptQuestionsTable)
            .where(and(eq(attemptQuestionsTable.attemptId, attemptId), eq(attemptQuestionsTable.questionOrder, order)))
            .limit(1);
        if (!aq)
            throw new APIError("NOT_FOUND", {
    ...
    // ── UPDATE VIEWED AT ─────────────────────────────────────────────────────
    updateViewedAt: async (attemptId, order, viewedAt = new Date()) => {
    ...
    // ── UPSERT ANSWER ────────────────────────────────────────────────────────
    upsertAnswer: async (attemptId, order, answerBody) => {
    ...
    submitAttemptWithAutoGrade: async (attemptId) => {
        ...
        const [attempt] = await db
            ...
            .limit(1);
        if (!attempt)
            throw new Error("Attempt not found"); // This should not happen if attemptRows is not empty
        const attemptedQuestionIds = attemptRows.map((r) => r.id);
        ...
        for (const answer of answerRows) {
            ...
            if (snapshot.type === "CHOICE") {
                if (answer.selectedChoiceId !== null) {
                    ...
                    const choice = snapshot.choices?.find((c) => c.id === answer.selectedChoiceId);
                ...
                if (answer.booleanAnswer !== null) {
                    ...
                    // If not found, we could look at choices as fallback
                        ...
                        const correctChoice = snapshot.choices.find((c) => c.isCorrect === true);
                        if (correctChoice) {
                            // Assume the correct choice's choiceText is "true" or "false" (lowercase)
                            const correctBool = correctChoice.choiceText?.toLowerCase() === "true";
