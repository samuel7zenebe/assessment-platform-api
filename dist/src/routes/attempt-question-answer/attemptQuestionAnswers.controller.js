...
export const submitAnswer = factory.createHandlers(sValidator("json", SubmitAnswerSchema), async (c) => {
    ...
    try {
        ...
        if (existing)
            throw new APIError("CONFLICT", { message: "Answer already submitted for this question", code: "409" });
...
export const getSavedAnswer = factory.createHandlers(async (c) => {
    const attemptQuestionId = c.req.param("id");
    if (!attemptQuestionId)
        throw new APIError(400, { message: "Missing attempt question id" });
    try {
        const answer = await attemptQuestionAnswerRepo.getSavedAnswer(attemptQuestionId);
        if (!answer)
            throw new APIError(404, { message: "No saved answer found for this question" });
...
export const getAllAttemptAnswers = factory.createHandlers(async (c) => {
    const attemptId = c.req.param("id");
    if (!attemptId)
        throw new APIError(400, { message: "Missing attempt id" });
    try {
...
export const updateAnswer = factory.createHandlers(sValidator("json", UpdateAnswerSchema), async (c) => {
    const answerId = c.req.param("id");
    if (!answerId)
        throw new APIError(400, { message: "Missing answer id" });
    try {
        const answer = await attemptQuestionAnswerRepo.updateAnswer(answerId, c.req.valid("json"));
        if (!answer)
            throw new APIError(404, { message: "Answer not found" });
...
export const gradeAnswer = factory.createHandlers(sValidator("json", GradeAnswerSchema), async (c) => {
    const answerId = c.req.param("id");
    if (!answerId)
        throw new APIError(400, { message: "Missing answer id" });
    try {
...
export const bulkGradeAnswers = factory.createHandlers(sValidator("json", BulkGradeSchema), async (c) => {
...
export const getAnswerStatistics = factory.createHandlers(async (c) => {
