...
import { examCandidates, examQuestions, examAttempts, attemptQuestions as attemptQuestionsTable, answers as answersTable, questionBank, questionChoices, exams, } from "@/src/db/schema.js";
...
export const examAttemptsRepo = {
    ...
    listAttempts: async (filters) => {
        ...
        if (filters.examId)
            candidateConditions.push(eq(examAttempts.examId, filters.examId));
        if (filters.candidateId)
            candidateConditions.push(eq(examAttempts.candidateId, filters.candidateId));
        if (filters.status) {
    ...
    getAttemptReview: async (attemptId) => {
        return await db
            ...
            .leftJoin(questionChoices, and(eq(questionChoices.questionId, questionBank.id), eq(questionChoices.isCorrect, true)))
            .where(eq(attemptQuestionsTable.attemptId, attemptId));
    },
    // ── START ───────────────────────────────────────────────────────────────────
    startExam: async (params) => {
        ...
        if (!exam)
            throw new APIError("NOT_FOUND", { message: "Exam not found", status: 404 });
        ...
        if (rawQuestions.length === 0) {
            throw new APIError("BAD_REQUEST", { message: "Exam has no questions. Please contact an administrator.", status: 400 });
        }
        ...
        const questionIds = allAQs.map((aq) => aq.questionId);
            ...
            questions: allAQs.map((aq) => {
                ...
                return {
                    ...
                    question: {
                        ...
                        audioUrl: q?.audioUrl,
                        videoUrl: q?.videoUrl,
                        choices: q?.type === "CHOICE" || q?.type === "TRUE_FALSE" ? choicesMap[q.id] : undefined,
                    },
                };
            }),
        };
    ...
    submitAttempt: async (attemptId) => {
        ...
        const totalPossibleRaw = (await db
            ...
            .from(questionBank)
            .where(inArray(questionBank.id, attemptRows.map((r) => r.questionId).filter(Boolean))))
            .at(0)?.count ?? 0;
    ...
    getResult: async (attemptId) => {
        const attemptRows = await db
            ...
            .where(eq(attemptQuestionsTable.attemptId, attemptId));
        const attemptedQuestionIds = attemptRows.map((r) => r.id);
        const questionIds = attemptRows.map((r) => r.questionId).filter(Boolean);
        const totalPossibleRaw = (await db
            .select({ count: sql `SUM(${questionBank.points})` })
            .from(questionBank)
            .where(inArray(questionBank.id, attemptRows.map((r) => r.questionId).filter(Boolean))))
