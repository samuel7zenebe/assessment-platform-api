...
export const clip = (n) => Math.round(n * 100) / 100;
// Convert a nullable sql/number/string to a number
export const num = (v) => typeof v === "number" ? v : parseFloat(v ?? "0");
...
export const analyticsRepo = {
    getExamSummary: async (examId) => {
        ...
        const passRate = gradedWithScore > 0 ? (passedAttempts / gradedWithScore) * 100 : 0;
        ...
        const completionRate = totalAttempts > 0 ? (startedAttempts / totalAttempts) * 100 : 0;
        return {
            examId,
            totalAttempts,
            startedAttempts,
