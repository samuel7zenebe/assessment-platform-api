...
import { z } from "zod";
import { ListExamCandidatesSchema, AssignCandidatesSchema, UnassignCandidateSchema, UpdateAssignmentStatusSchema, } from "./schema.js";
import { APIError } from "better-auth";
import { hasPermission } from "@/src/middleware/auth.js";
const factory = createFactory();
// ── GET  /exams/:examId/candidates               → list assigned candidates ─
export const listCandidates = factory.createHandlers(hasPermission({
    resource: "candidate",
    action: "list",
}), sValidator("param", z.object({ examId: z.uuid() })), async (c) => {
...
export const assignCandidates = factory.createHandlers(hasPermission({
    resource: "candidate",
    action: "assign_exam",
}), sValidator("param", z.object({ examId: z.uuid() })), sValidator("json", AssignCandidatesSchema), async (c) => {
    const examId = c.req.valid("param").examId;
...
export const updateAssignmentStatus = factory.createHandlers(hasPermission({
    resource: "candidate",
    action: "assign_exam",
}), sValidator("param", z.object({
    examId: z.uuid(),
    candidateId: z.uuid(),
})), sValidator("json", UpdateAssignmentStatusSchema), async (c) => {
    const { examId, candidateId } = c.req.valid("param");
    const { assignmentStatus } = c.req.valid("json");
    try {
        const existing = await examCandidatesRepo.findAssignment(examId, candidateId);
        if (!existing)
            throw new APIError("NOT_FOUND", {
                message: "Assignment not found",
                status: 404,
            });
        const updated = await examCandidatesRepo.updateStatus(examId, candidateId, { assignmentStatus });
        return c.json({
            data: updated,
            success: true,
...
export const unassignCandidate = factory.createHandlers(hasPermission({
    resource: "candidate",
