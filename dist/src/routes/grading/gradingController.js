...
import { GradingQueueQuerySchema, GradeAnswerBodySchema, FinalizeGradingSchema, } from "./schma.js";
const factory = createFactory();
// ── GET   /api/grading/queue                # Attempts with status=SUBMITTED needing manual review
export const getGradingQueue = factory.createHandlers(sValidator("query", GradingQueueQuerySchema), async (c) => {
...
export const getAttemptForReview = factory.createHandlers(sValidator("param", z.object({ attemptId: z.uuid() })), async (c) => {
...
export const updateAnswerGrading = factory.createHandlers(sValidator("param", z.object({
    attemptId: z.uuid(),
    answerId: z.uuid(),
})), sValidator("json", GradeAnswerBodySchema), async (c) => {
    ...
    try {
        const updatedAnswer = await gradingRepo.updateAnswerGrading(attemptId, answerId, { isCorrect, awardedPoints, reviewerFeedback });
...
export const finalizeGrading = factory.createHandlers(sValidator("param", z.object({ attemptId: z.uuid() })), sValidator("json", FinalizeGradingSchema), async (c) => {
    const { attemptId } = c.req.valid("param");
    const userId = c.get("user").id;
    const role = c.get("user").role;
