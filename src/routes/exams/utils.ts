...
import type { QuestionBankSchema } from "../questions/schema.js";

type jobTitleProps = Array<{
  id: string;
  weight: number;
}>;
type GenerateExamProps = {
  totalQuestions: number;
  difficultyLevel: number;
  jobTitles: jobTitleProps;
};

export async function generateExamByDifficulty({
  difficultyLevel,
  jobTitles,
  totalQuestions,
}: GenerateExamProps) {
...
function calculateQuestionDistribution(
  ...
  if (sum !== totalQuestions) {
    ...
    if (hardCount > mediumCount && hardCount > easyCount) hardCount += diff;
    else if (mediumCount > easyCount) mediumCount += diff;
    else easyCount += diff;
  }
