...
export const getCandidateAnalytics = factory.createHandlers(
  sValidator("param", z.object({ id: z.string() })),
  sValidator("query", CandidateAnalyticsSchema),
  async (c) => {
    const { id } = c.req.valid("param");
    const userId = c.get("user").id;
    const role = c.get("user").role;
...
export const getPlatformOverview = factory.createHandlers(
  sValidator("query", PlatformOverviewSchema),
  async (c) => {
    const userId = c.get("user").id;
    const role = c.get("user").role;
