...
export const listAttempts = factory.createHandlers(
  sValidator("query", ListAttemptsQuerySchema),
  async (c) => {
...
export const getAttempt = factory.createHandlers(
...
export const startExam = factory.createHandlers(
  sValidator("param", z.object({ examId: z.uuid() })),
  sValidator("json", StartExamSchema),
  async (c) => {
...
export const submitAttempt = factory.createHandlers(
  sValidator("param", AttemptIdParamSchema),
  sValidator("json", SubmitAttemptSchema),
  async (c) => {
    const { attemptId } = c.req.valid("param");
    try {
...
export const expireAttempt = factory.createHandlers(
  sValidator("param", AttemptIdParamSchema),
  async (c) => {
    const { attemptId } = c.req.valid("param");
    try {
...
export const getAttemptResult = factory.createHandlers(
  sValidator("param", AttemptIdParamSchema),
  async (c) => {
    const { attemptId } = c.req.valid("param");
    try {
...
export const getAttemptReview = factory.createHandlers(
  sValidator("param", AttemptIdParamSchema),
  sValidator("query", AttemptReviewQuerySchema),
  async (c) => {
    const { attemptId } = c.req.valid("param");
    try {
